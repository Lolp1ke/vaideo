...
for i in range(1,num_pic):
    ...
    if np.max(time_delay)/frame_rate>len_time_thre and start == 0: #and score_matrix[index]/count_matrix[index]>0.8:
        ...
        if 'start_time' in anomely_now and (time_frame/frame_rate - anomely_now['end_time'])<30:
            ...
            feature1 = reid_model.extract("data/mediadata/ori_images/%s/%d.jpg"%(a,max(1,anomely_now['start_time']*frame_rate)),anomely_now['region'])
            feature2 = reid_model.extract("data/mediadata/ori_images/%s/%d.jpg"%(a,max(1,time_frame)),region)
        ...
        tmp_im = cv2.imread("data/mediadata/ori_images/%s/%d.jpg"%(a,time_frame))
