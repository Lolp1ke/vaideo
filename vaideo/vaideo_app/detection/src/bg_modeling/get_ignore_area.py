...
gass_sigma = 3
score_thred = 0.1
dt_results_fbf = {}  # Added initialization of dt_results_fbf
with open("detection_results/test_framebyframe/video%s.txt" % video_id,'r') as f:
    for line in f:
        ...
        if score > score_thred:
            dt_results_fbf[frame].append([x1, y1, x1+tmp_w, y1+tmp_h, score])  # Completed conditional statement

im = cv2.imread("data/mediadata/ori_images/%s/1.jpg" % video_id)
h, w, c = im.shape
mat = np.zeros((h, w))
